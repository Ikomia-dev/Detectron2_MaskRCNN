...
# --------------------
class Detectron2_MaskRCNNWidget(core.CProtocolTaskWidget):

    def __init__(self, param, parent):
        core.CProtocolTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = processMod.Detectron2_MaskRCNNParam()
        else:
...
#--------------------
class Detectron2_MaskRCNNWidgetFactory(dataprocess.CWidgetFactory):

    def __init__(self):
        dataprocess.CWidgetFactory.__init__(self)
        # Set the name of the process -> it must be the same as the one declared in the process factory class
        self.name = "Detectron2_MaskRCNN"

    def create(self, param):
        # Create widget object
        return Detectron2_MaskRCNNWidget(param, None)
